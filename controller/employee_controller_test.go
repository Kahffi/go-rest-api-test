...
func setupTestAppEmployee(mockService *mocks.MockEmployeeService) *fiber.App {
	...
	api := app.Group("/api")
	emplooyees := api.Group("/emplooyees")
	emplooyees.Post("/", employeeController.Create)
	emplooyees.Put("/:employeeId", employeeController.Update)
	emplooyees.Delete("/:employeeId", employeeController.Delete)
	emplooyees.Get("/:employeeId", employeeController.FindById)
	emplooyees.Get("/", employeeController.FindAll)
